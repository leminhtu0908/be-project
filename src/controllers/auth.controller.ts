...
import { SocialProvider } from "../configs/passport.configs";
...
const AuthController = {
  ...
  signUp: async (
    ...
    const token = jwt.sign(
      ...
      process.env.SECRET as string as string,
      { expiresIn: "1h" }
    );
  ...
  //       return res
  //         .status(ErrorCodes.Bad_Request)
  //         .send("Email verification has been failed due to invalid arguments.");
  //     }

  //     let decoded = null;
  //     try {
  //       decoded = await jwt.verify(token, process.env.SECRET as string);
  //     } catch (error) {
  ...
  //       return res
  //         .status(ErrorCodes.Bad_Request)
  //         .send("Email is already verified.");
  //     }
  ...
  forgotPassword: async (req: Request, res: Response): Promise<any> => {
    ...
    const token = jwt.sign({ user: { email } }, process.env.SECRET as string, {
      expiresIn: "1h",
    });
